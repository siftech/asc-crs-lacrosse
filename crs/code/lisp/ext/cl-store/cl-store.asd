...
(unless (eq :xy :XY)
  (error
   "CL-STORE requires case-insensitive Lisp but (eq :xy :XY) returns nil"))
;; Don't know about this one.
;;;(let ((check (symbol-name '#:xy)))
;;;  (unless (equal "XY" check)
;;;    (error
;;;     "CL-STORE requires default upper-case symbols ~
;;;        but (symbol-name '#:xy) returns ~s not ~s" check "XY")))
