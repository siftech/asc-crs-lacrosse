...
BOUND)."
  ...
  (lambda ()
    (let* ((most-negative (ecase type
      ...
      (
       ;;   There's a bug in the ACL 8.2 64bit 
       ;; compiler that gets tickled when this is compiled with (speed 3) (safety 0).
       ;; (Credit for workaround to Franz's Duane Rettig.)
       #-(and allegro-version>= (version>= 8 2) 64bit)
         progn
       #+(and allegro-version>= (version>= 8 2) 64bit)
         excl::compiler-let   
       #+(and allegro-version>= (version>= 8 2) 64bit)
         ((comp::trust-table-case-argument-switch nil))
