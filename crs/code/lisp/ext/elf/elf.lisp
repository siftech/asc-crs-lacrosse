...
;;
;;
;; For the CGC program's CGCEF format
;; -------------------------------------------------
;; Use variations cgcef-p, read-cgcef, write-cgcef
;;
...
;;; Code:
(in-package :elf)
(debug-list-value :elf "ELF (and CGCEF) binary manipulation")
(pushnew :elf *debug-list*)

(debug-list-value :elf-deep "Deep ELF (and CGCEF) binary manipulation")

(defvar *cgcef-mode* nil
  "Flag for the CGCEF format mode for DARPA CGC. If non-nil, we're working
with CGCEF files, or want to pretend we are. If :STRIP, then we want to write
CGCEF binaries with the symbol header table stripped out.")

;;; Basic Binary types
;; strings
...
(defun dbug-elf-header (header)
  (dbug :elf-deep "------ elf-header start")
  (dbug :elf-deep "magic-number ~s" (magic-number header))
  (dbug :elf-deep "file-class ~s" (file-class header))
  (dbug :elf-deep "data-encoding ~s" (data-encoding header))
  (dbug :elf-deep "ident-version ~s" (ident-version header))
  (dbug :elf-deep "padding ~s" (padding header))
  (dbug :elf-deep "ei-size ~s" (ei-size header))
  (dbug :elf-deep "type ~s" (type header))
  (dbug :elf-deep "machine ~s" (machine header))
  (dbug :elf-deep "version ~s" (version header))
  (dbug :elf-deep "entry ~s" (entry header))
  (dbug :elf-deep "phoff ~s" (phoff header))
  (dbug :elf-deep "shoff ~s" (shoff header))
  (dbug :elf-deep "flags ~s" (flags header))
  (dbug :elf-deep "eh-size ~s" (eh-size header))
  (dbug :elf-deep "ph-ent-size ~s" (ph-ent-size header))
  (dbug :elf-deep "ph-num ~s" (ph-num header))
  (dbug :elf-deep "sh-ent-size ~s" (sh-ent-size header))
  (dbug :elf-deep "sh-num ~s" (sh-num header))
  (dbug :elf-deep "sh-str-ind ~s" (sh-str-ind header))
  (dbug :elf-deep "------ elf-header end"))

(defmethod write-value :around ((type (eql 'elf-header)) out value
                                &key &allow-other-keys)
  "Implement the ELF/CGCEF-header-relevant parts of the *CGCEF-MODE* switch."
  (declare (ignore out))
  (cond
    ((eq *cgcef-mode* :strip)
     (let ((shoff (shoff value))
           (sh-num (sh-num value))
           (sh-str-ind (sh-str-ind value)))
       (setf (shoff value) 0
             (sh-num value) 0
             (sh-str-ind value) 0)
       (call-next-method)
       (setf (shoff value) shoff
             (sh-num value) sh-num
             (sh-str-ind value) sh-str-ind)))
    (t (call-next-method))))
...
(defclass elf ()
  ((header        :initarg :header        :accessor header)
   ...
   ;; This tells us whether the section table was inferred, rather than parsed, from the binary.
   (section-table-inferred :initarg :section-table-inferred :accessor section-table-inferred :initform nil)
   ))

(defun program-headers-for-section (sec)
  (remove-if-not #'(lambda (ph)
                     (and (>= (offset sec) (offset ph))
                          (<= (+ (offset sec) (size sec))
                              (+ (offset ph) (filesz ph)))))
                 (elf::program-table (elf::elf sec))))
  ...
  (flet ((raw-bytes (from size)
    ...
    (let ((e (make-instance 'elf)))
      (with-slots (header section-table program-table sections ordering) e
        ...
        (when-dbug :elf-deep
                   (dbug-elf-header header)
                   (dbug :elf-deep "Stream after header ~s" in))
        (setf program-table
              ...
              (unless (zerop (shoff header))
                (progn
                  (file-position in (shoff header))
                  (loop for x from 0 to (1- (sh-num header))
                      collect (let ((section-hdr
                                     (read-value 'section-header in)))
                                (dbug :elf-deep "Read section header ~d" x)
                                (dbug :elf-deep "at ~a:" in)
                                section-hdr)))))
      ...
      (mapc (lambda-bind ((offset . bytes))
            ...
            (mapcar
             (lambda-bind ((offset size data))
               ...
               ;; (dbug :elf-deep "data ~s to ~d" data offset)
               (cons
                offset
                (cond
                  ...
                  ((eq :header data)        ; header
                   (when-dbug :elf-deep (dbug-elf-header header))
                   (bytes (write-value 'elf-header o header)))
                  ((eq :section-table data) ; section table
                   (cond
                     ((eq *cgcef-mode* :strip)
                      (dbug :elf-deep
                            "CGCEF-WRITE :strip so skip section header write"))
                     (t
                      (dbug :elf-deep "CGCEF-WRITE writing sections")
                      (bytes (mapc (lambda (c)
                                     (dbug :elf-deep "~s" c)
                                     ;; (dbug :elf-deep "at ~s" o)
                                     (write-value 'section-header o c))
                                   section-table)))))
                  ((eq :program-table data) ; program table
                   (bytes (mapc (lambda (c) (write-value (program-header-type) o c))
                                program-table)))
                  ((vectorp data) data) ; raw filler
                  ((eql data :end) nil) ; acceptable no-op
                  (t (error "Out the bottom of write-value cond: ~s" data)))))
             ordering))))
...
(defun cgcef-p (file)
  "Return t if file is a CGCEF file (using the magic number test)."
  (with-open-file (in file :element-type '(unsigned-byte 8))
    (string= (concatenate 'string (string (code-char #x7f)) "CGC")
             (read-value 'string in :length 4))))
...
(defun write-elf (elf file)
  (let ((given-magic (magic-number (header elf))))
    (setf (magic-number (header elf))
          (concatenate 'string (string (code-char #x7f)) "ELF"))
    (with-open-file (out file :direction :output :element-type '(unsigned-byte 8))
      (write-value 'elf out elf))
    (setf (magic-number (header elf)) given-magic))
  nil)

(defun read-cgcef (file)
  (let ((*cgcef-mode* t))
    (with-open-file (in file :element-type '(unsigned-byte 8))
      (read-value 'elf in))))

(defun write-cgcef (elf file &key strip-section-headers
                               (if-exists :error)) ; Same default as with-open-file
  (let ((*cgcef-mode* (if strip-section-headers :strip t))
        (given-magic (magic-number (header elf))))
    (setf (magic-number (header elf))
          (concatenate 'string (string (code-char #x7f)) "CGC"))
    (with-open-file (out file :direction :output :element-type '(unsigned-byte 8) :if-exists if-exists)
      (write-value 'elf out elf))
    (setf (magic-number (header elf)) given-magic))
  nil)

(defun cgcef-to-elf (bin)
  "Read BIN, write it out to BIN.elf."
  (write-elf (read-elf bin) (concatenate 'string bin ".elf")))
...
(defparameter *objdump-formatter*
  #-macosx "objdump -j ~a -d ~a"
  #+macosx "gobjdump -j ~a -d ~a")

(defparameter *objdump-summary-formatter*
  "tools/summarize-objdump.pl objdump -j ~a -d ~a")
...
(defmethod objdump ((sec section))
  (with-temp-file path
    ...
    (shell (format nil *objdump-summary-formatter* (name sec) path))))

;;; Depending on whether we need to parse objdump and gobjdump output,
;;; we break up instruction lines at different columns.
(defconstant +colon-col+ #-macosx 8 #+macosx 12)
(defconstant +bytes-start-col+ #-macosx 10 #+macosx 14)
(defconstant +assembler-start-col+ #-macosx 32 #+macosx 36)

(defun parse-one-address-line (line)

  ;; Extract string fields
  (let ((memory-address (subseq line 1 +colon-col+))
        (bytes-sequence (trim (subseq line +bytes-start-col+ (if (> (length line) +assembler-start-col+) (- +assembler-start-col+ 1) nil))))
        (assembly-text (when (> (length line) (- +assembler-start-col+ 1)) (trim (subseq line +assembler-start-col+)))))

    ;; Convert fields to numbers and lists of numbers.
    (list (parse-integer memory-address :radix 16)

          ;; bytes
          (mapcar (lambda (num) (parse-integer num :radix 16))
                  (split-sequence #\Space bytes-sequence))

          ;; disassembled assembly text
          assembly-text)))

(defun line-is-relevant (line)
  (or (<= (length line) +colon-col+)
      (not (equal #\: (aref line +colon-col+)))))

(defun parse-addresses (lines)
  "Parse addresses from lines of objdump output."
  (let* ((relevant-lines (remove-if #'line-is-relevant lines))
         (parsed (mapcar #'parse-one-address-line relevant-lines)))
    parsed))
...
(defun objdump-parse (output)
  ...
  (let* ((lines (split-sequence #\Newline output))
         ...
         (header-mapped (mapcar sec-header lines))

         (instr-lines (cdr (split-sequence-if sec-header lines)))
         (parsed-instrs (mapcar #'parse-addresses instr-lines)))

    (mapcar #'cons (remove nil header-mapped) parsed-instrs)))
